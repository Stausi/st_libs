---@class StCommandParams
---@field name string
---@field help? string
---@field type? 'number' | 'playerId' | 'string' | 'longString'
---@field optional? boolean

---@class StCommandProperties
---@field help string?
---@field params StCommandParams[]?
---@field restricted boolean | string | string[]?

---@type StCommandProperties[]
...
---@param properties StCommandProperties | false
---@param cb fun(source: number, args: table, raw: string)
---@param ... any
function st.addCommand(commandName, properties, cb, ...)
    ...
    if properties then
        if ... or table.type(properties) ~= 'hash' then
            ...
            warn(("command '%s' is using deprecated syntax for st.addCommand\nupdate the command or use st.__addCommand to ignore this warning\n> source ^0(^5%s^0:%d)"):format(_commandName, info.short_src, info.currentline))
            ---@diagnostic disable-next-line: deprecated
            return st.__addCommand(commandName, properties, cb, ...)
        end
