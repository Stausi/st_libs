...
function st.raycast.fromCoords(coords, destination, flags, ignore)
...
function st.raycast.fromCamera(flags, ignore, distance)
    local coords = GetFinalRenderedCamCoord()
    local destination = coords + getForwardVector() * (distance or 10)

    return st.raycast.fromCoords(GetFinalRenderedCamCoord(), destination, flags, ignore)
end
