...
local function getModuleInfo(modName)
    ...
    while true do
        ...
        if resource and not src:find('^@@st_libs/modules/require') then
...
table.insert(package.searchers, function(modName)
    if loaded[modName] ~= nil and loaded[modName] ~= '__loading' then
        return function() 
            return loaded[modName]
        end
    end

    local resource, pathName = getModuleInfo(modName:gsub('%.', '/'))

    local baseDir = "modules/" .. pathName
    local tried   = {}

    local sharedFile = LoadResourceFile(resource, baseDir .. "/shared.lua")
    if sharedFile then
        local chunk, loadErr = load(
            sharedFile,
            ("@@%s/%s"):format(resource, baseDir .. "/shared.lua"),
            't',
            _ENV
        )
        if chunk then
            local ok, errOrRet = pcall(chunk)
            if not ok then
                tried[#tried+1] = ("error running shared.lua: %s"):format(errOrRet)
            end
        else
            tried[#tried+1] = ("failed to load shared.lua: %s"):format(loadErr)
        end
    else
        tried[#tried+1] = ("no file '@%s/%s'"):format(resource, baseDir .. "/shared.lua")
    end

    if IsDuplicityVersion and IsDuplicityVersion() then
        local serverFile = LoadResourceFile(resource, baseDir .. "/server.lua")
        if serverFile then
            return assert(load(
                serverFile,
                ("@@%s/%s"):format(resource, baseDir .. "/server.lua"),
                't',
                _ENV
            ))
        else
            tried[#tried+1] = ("no file '@%s/%s'"):format(resource, baseDir .. "/server.lua")
        end
    else
        local clientFile = LoadResourceFile(resource, baseDir .. "/client.lua")
        if clientFile then
            return assert(load(
                clientFile,
                ("@@%s/%s"):format(resource, baseDir .. "/client.lua"),
                't',
                _ENV
            ))
        else
            tried[#tried+1] = ("no file '@%s/%s'"):format(resource, baseDir .. "/client.lua")
        end
    end
