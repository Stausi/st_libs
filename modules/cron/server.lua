...
---@field job fun(task: StTask, date: osdate)
...
---@class StTask : StTaskProperties
---@field expression string
---@field private scheduleTask fun(self: StTask): boolean?
local StTask = {}
StTask.__index = StTask
...
function StTask:getNextTime()
    ...
    if self.lastRun and nextTime - self.lastRun < 60 then
        if self.debug then
            st.print.debug(('Preventing duplicate execution of task %s - Last run: %s, Next scheduled: %s'):format(
                self.id,
...
---@return number
function StTask:getAbsoluteNextTime()
    local minute = getTimeUnit(self.minute, 'min')
    local hour = getTimeUnit(self.hour, 'hour')
    local day = getTimeUnit(self.day, 'day')
    local month = getTimeUnit(self.month, 'month')
...
function StTask:getTimeAsString(timestamp)
...
function StTask:scheduleTask()
    ...
    if sleep < 0 then
        ...
        if self.debug then
            st.print.debug(('Task %s is %s seconds overdue, executing now due to maxDelay=%s'):format(
                self.id,
                -sleep,
                self.maxDelay
            ))
    ...
    if self.debug then
        st.print.debug(('(%s) task %s will run in %d seconds (%0.2f minutes / %0.2f hours)'):format(timeAsString, self.id, sleep,
            sleep / 60,
            sleep / 60 / 60))
    end
    ...
    if self.isActive then
        if self.debug then
            st.print.debug(('(%s) running task %s'):format(timeAsString, self.id))
        end
...
function StTask:stop(msg)
    self.isActive = false

    if self.debug then
        if msg then
            return st.print.debug(('stopping task %s (%s)'):format(self.id, msg))
        end

        st.print.debug(('stopping task %s'):format(self.id))
    end
end

---@param expression string A cron expression such as `* * * * *` representing minute, hour, day, month, and day of the week.
---@param job fun(task: StTask, date: osdate)
...
function st.cron.new(expression, job, options)
    ...
    ---@type StTask
    local task = setmetatable(options or {}, StTask)
